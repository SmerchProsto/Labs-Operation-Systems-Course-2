...
namespace DisableDevice
{
    ...
    public sealed class DeviceHelper
    {
        ...
        /// </резюме>
        /// < /// <param name="Руководство по классам">Идентификатор класса устройства. Доступно в диспетчере устройств.</param>
        /// <param name="InstanceID">Идентификатор экземпляра устройства устройства. Доступно в диспетчере устройств.</param>
        /// <param name="enable">True для включения, False для отключения.</param>
        /// <примечания>Вызовет исключение, если устройство не отключено.</примечания>
        public static void SetDeviceEnabled(Guid classGuid, string instanceId, bool enable)
        {
            SafeDeviceInfoSetHandle diSetHandle = null;
            try
            {
                // Получить дескриптор набора сведений об устройстве для всех устройств, соответствующих ClassGUID, которые присутствуют в системе
                //.
                diSetHandle = NativeMethods.SetupDiGetClassDevs(ref classGuid, null, IntPtr.Zero, SetupDiGetClassDevsFlags.Present);
               // Получить данные об устройстве для каждого соответствующего устройства.
                DeviceInfoData[] diData = GetDeviceInfoData(diSetHandle);
                // Найдите индекс нашего экземпляра, т. Е. мышь с тачпадом - у меня есть 3 мыши...
        ...
        // // Найдите индекс конкретного DeviceInfoData для идентификатора экземпляра.
        ...
        private static void EnableDevice(SafeDeviceInfoSetHandle handle, DeviceInfoData diData, bool enable)
        {
            ...
            // Размер-это просто размер заголовка, но мы выровняли структуру.
            // Заголовок содержит первые два поля, оба целочисленные.
            @params.Size = 8;
